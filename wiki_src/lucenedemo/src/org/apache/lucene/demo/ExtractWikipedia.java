...
public class ExtractWikipedia {
  ...
  public void create(String id, String title, String time, String body) {
   ...
   // File f = new File(d, id + ".txt");
    ...
    try {
      ...
      String wikicontent = wikiModel.render(new PlainTextConverter(), contents.toString());      
  ...
  public ExtractWikipedia(String xmldocument, String docsPath) throws Exception{
    File wikipedia = null;
    File outputDir = new File(docsPath);
    boolean keepImageOnlyDocs = true;
   //for (int i = 0; i < args.length; i++) {
     // String arg = args[i];
      //if (arg.equals("--input") || arg.equals("-i")) {
    	  
    wikipedia = new File(xmldocument);
